...
def process_message(ch, method, properties, body):
    global processed_msgs, neighbors, val, bits, return_node, vectors, active, start, end, isdead
    ...
    if decoded_body == 'start':
        ...
        if message_type == 'broadcast':
            ...
            new_bits = [[bit1 | bit2 for bit1, bit2 in zip(sublist1, sublist2)] for sublist1, sublist2 in zip(bits, value)]
            if(new_bits != bits):
                bits = new_bits
                for node_dest in neighbors:
                    send_message('convergecast', bits, node_dest)
